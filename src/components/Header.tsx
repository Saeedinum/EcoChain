  ...
  return (
    <header className="m-5 flex h-[75px] items-center justify-between rounded-[50px] py-5 md:mx-10 md:mb-10">
      ...
      <div className="sm:hidden">
        <DropdownMenu>
          <DropdownMenuTrigger>
            <img src={menuLogo} alt="nav links" className="border-none outline-none" />
          </DropdownMenuTrigger>
          <DropdownMenuContent>
            <DropdownMenuItem>
            ...
            {auth.token ? (
              <div className="place-self-center text-[20px] text-[#0B539B]">{auth.firstName}</div>
            ) : (
              <div className="itmes-center flex w-full flex-col justify-center">
                <Link to={"/login"} className="text-[20px] font-bold text-[#0B539B]">
                  Login
                </Link>
                <Link to={"/signup"} className="text-[20px] font-bold text-[#0B539B]">
                  Sign up
                </Link>
              </div>
            )}
